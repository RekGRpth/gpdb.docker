...
docker run \
    ...
    --mount type=bind,source="$(docker volume inspect --format "{{ .Mountpoint }}" gpdb)/.local5",destination=/usr/local \
    --mount type=bind,source=/tmpfs/data5,destination=/tmpfs/data5 \
    --name gpdb5 \
