...
docker stop "gpdb${GP_MAJOR}u" || echo $?
docker rm "gpdb${GP_MAJOR}u" || echo $?
...
docker run \
    ...
    --env USER_ID="$(id -u)" \
    --hostname "gpdb${GP_MAJOR}u" \
    --init \
    --memory=16g \
    --memory-swap=16g \
    --mount type=bind,source="$(docker volume inspect --format "{{ .Mountpoint }}" gpdb)/.local/${GP_MAJOR}u",destination=/usr/local \
    ...
    --name "gpdb${GP_MAJOR}u" \
    --network name=docker \
    --privileged \
    --restart always \
    --sysctl "kernel.sem=500 1024000 200 4096" \
    "gpdb${GP_MAJOR}u" sudo /usr/sbin/sshd -De
